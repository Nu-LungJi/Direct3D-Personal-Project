...
class Monster_Knight : public GameObject {
private:
	Monster_Knight(ComPtr<ID3D11Device>	_GRPDEV, ComPtr<ID3D11DeviceContext> _DEVCTX);
	Monster_Knight(CONST Monster_Knight& _PRTOBJ);
public:
	virtual ~Monster_Knight();
...
public:
	static	unique_ptr<Monster_Knight>		Create(ComPtr<ID3D11Device> _GRPDEV, ComPtr<ID3D11DeviceContext> _DEVCTX);
...
private:
	...
	shared_ptr<BehaviorTree_Knight>		BehaviorTree	= { nullptr };
