...
private:
	PlayerEffect(ComPtr<ID3D11Device>	_GRPDEV, ComPtr<ID3D11DeviceContext> _DEVCTX);
	PlayerEffect(CONST PlayerEffect& _PRTOBJ);
public:
...
public:
	...
	VOID				Descend_ScrollSpeed(CONST _float& _DT);
...
public:
	...
	VOID	Set_EffectOption(uint32_t _OptionIndex) { EffectOption = _OptionIndex; }
	...
	VOID		Set_ShaderPassIndex(uint32_t _PassIndex) { ShaderPassIndex = _PassIndex; }
public:
	static	unique_ptr<PlayerEffect>	Create(ComPtr<ID3D11Device> _GRPDEV, ComPtr<ID3D11DeviceContext> _DEVCTX);
...
private:
	...
	shared_ptr<class Player>			GamePlayer				= { nullptr };
	...
	XMFLOAT3     EffectEmissiveColor = { 1.f, 0.f, 0.f };
	_float       EffectEmissiveIntensity = { 1.f };
