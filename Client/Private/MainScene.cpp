...
MainScene::MainScene(ComPtr<ID3D11Device> _GRPDEV, ComPtr<ID3D11DeviceContext> _DEVCTX) : Scene(_GRPDEV, _DEVCTX) { }
MainScene::~MainScene() {}
HRESULT MainScene::Initialize() {

	return S_OK;
}
HRESULT		MainScene::Register_ProtoType() {
	...
	Add_ProtoType_CurrentScene("PRT_Ocean"				, Ocean				::Create(GRPDEV, DEVCTX));
	...
	GameInstance::GetInstance().Get_LightManager()->Generate_Light(LIGHT_TYPE::DIRECTIONAL, _float4(1.f, 1.f, 1.f, 1.f), _float4(0.2f, 0.2f, 0.2f, 1.f), _float4(0.3f, 0.3f, 0.3f, 1.f), _float4(1.f, -1.f, 1.f, 0.f),
		_float4(0.f, 0.f, 0.f, 0.f), 0.f);
	//GameInstance::GetInstance().Get_LightManager()->Generate_Light(LIGHT_TYPE::POINT, _float4(1.f, 0.4f, 0.4f, 1.f), _float4(0.4f, 0.2f, 0.2f, 1.f), _float4(1.f, 0.4f, 0.4f, 1.f), _float4(1.f, -1.f, 1.f, 0.f),
	//	_float4(20.f, 5.f, 20.f, 1.f), 20.f);
	//GameInstance::GetInstance().Get_LightManager()->Generate_Light(LIGHT_TYPE::POINT, _float4(0.4f, 1.f, 0.4f, 1.f), _float4(0.2f, 0.4f, 0.2f, 1.f), _float4(0.4f, 1.f, 0.4f, 1.f), _float4(1.f, -1.f, 1.f, 0.f),
	//	_float4(40.f, 5.f, 20.f, 1.f), 20.f);

	return S_OK;
}
HRESULT		MainScene::Initialize_SceneObject() {
	...
	Add_CloneObject((uint32_t)LAYER_TYPE::LAYER_1, "PRT_Ocean"				, "Ocean"				, nullptr);
...
HRESULT		MainScene::Initialize_MapObject() {
	...
	//GameInstance::GetInstance().Get_MapEditor()->Load_WorldMap_ObjectList("../../Resource/WorldMapData/LandScapeData/", (uint32_t)SCENE_TYPE::SCENE_1, (uint32_t)LAYER_TYPE::LAYER_LANDSCAPE);

	return S_OK;
}
VOID	MainScene::Update(CONST _float& _DT) {
	...
	if (KEY_DOWN(DIK_TAB))									{ GameInstance::GetInstance().Get_Navigator()->Select_NextRayCastedObject();	}

	if (KEY_HOLD(DIK_LCONTROL) && KEY_HOLD(DIK_LSHIFT) && KEY_DOWN(DIK_F12)) {
		if (6 == MessageBoxA(hWnd, "Save WorldMap Data", "Save Data?", MB_YESNO)) {
			GameInstance::GetInstance().Get_MapEditor()->Save_WorldMap_ObjectList("../../Resource/WorldMapData/BuildingData/"	, (uint32_t)SCENE_TYPE::SCENE_1, (uint32_t)LAYER_TYPE::LAYER_BUILDING);
			GameInstance::GetInstance().Get_MapEditor()->Save_WorldMap_ObjectList("../../Resource/WorldMapData/MapObjectData/", (uint32_t)SCENE_TYPE::SCENE_1, (uint32_t)LAYER_TYPE::LAYER_MAPOBJECT);
			GameInstance::GetInstance().Get_MapEditor()->Save_WorldMap_ObjectList("../../Resource/WorldMapData/LandScapeData/", (uint32_t)SCENE_TYPE::SCENE_1, (uint32_t)LAYER_TYPE::LAYER_LANDSCAPE);
			GameInstance::GetInstance().Get_MapEditor()->Save_WorldMap_ObjectList("../../Resource/WorldMapData/FoliageData/", (uint32_t)SCENE_TYPE::SCENE_1, (uint32_t)LAYER_TYPE::LAYER_FOLIAGE);
	...
	if (KEY_HOLD(DIK_LCONTROL) && KEY_HOLD(DIK_LSHIFT) && KEY_DOWN(DIK_F11)) {
		if (6 == MessageBoxA(hWnd, "Load WorldMap Data", "Load Data?", MB_YESNO)) {
			GameInstance::GetInstance().Get_MapEditor()->Load_WorldMap_ObjectList("../../Resource/WorldMapData/BuildingData/", (uint32_t)SCENE_TYPE::SCENE_1, (uint32_t)LAYER_TYPE::LAYER_BUILDING);
			GameInstance::GetInstance().Get_MapEditor()->Load_WorldMap_ObjectList("../../Resource/WorldMapData/MapObjectData/", (uint32_t)SCENE_TYPE::SCENE_1, (uint32_t)LAYER_TYPE::LAYER_MAPOBJECT);
			GameInstance::GetInstance().Get_MapEditor()->Load_WorldMap_ObjectList("../../Resource/WorldMapData/LandScapeData/", (uint32_t)SCENE_TYPE::SCENE_1, (uint32_t)LAYER_TYPE::LAYER_LANDSCAPE);
			GameInstance::GetInstance().Get_MapEditor()->Load_WorldMap_ObjectList("../../Resource/WorldMapData/FoliageData/", (uint32_t)SCENE_TYPE::SCENE_1, (uint32_t)LAYER_TYPE::LAYER_FOLIAGE);
	...
	if (GameInstance::GetInstance().Get_SceneOption().CurrentSceneIndex != static_cast<uint32_t>(SCENE_TYPE::SCENE_1)) {
		GameInstance::GetInstance().Get_CollisionManager()->Reset_Recipient();
		GameInstance::GetInstance().Get_SceneManager()->Reset_Scene(static_cast<uint32_t>(SCENE_TYPE::SCENE_1));
		GameInstance::GetInstance().Get_Navigator()->Reset_Navigator();
		GameInstance::GetInstance().Get_LightManager()->Reset_LightManager();
		GameInstance::GetInstance().Get_RayCaster()->Reset_RayCasterRecipient();

		if (FAILED(GameInstance::GetInstance().Get_SceneManager()->Get_CurrentScene()->Register_ProtoType()))		return;	// 씬 초기화(오브젝트 클론)
		if (FAILED(GameInstance::GetInstance().Get_SceneManager()->Get_CurrentScene()->Initialize_SceneObject()))	return ;	// 씬 초기화(오브젝트 클론)
		if (FAILED(GameInstance::GetInstance().Get_SceneManager()->Get_CurrentScene()->Initialize_MapObject()))		return ;	// 맵 오브젝트 초기화
	}
 }
shared_ptr<MainScene>	MainScene::Create(ComPtr<ID3D11Device> _GRPDEV, ComPtr<ID3D11DeviceContext> _DEVCTX) {
	auto	SceneInstance = shared_ptr<MainScene>(new MainScene(_GRPDEV, _DEVCTX));
	if (FAILED(SceneInstance->Initialize())) {
		MSG_BOX("Failed to Created : MainScene");
		return nullptr;
	}
	return SceneInstance;
}
shared_ptr<GameObject>	MainScene::Clone(VOID* _ARG) {
	return nullptr;
}