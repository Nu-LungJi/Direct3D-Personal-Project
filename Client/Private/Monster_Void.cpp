...
Monster_Void::Monster_Void(ComPtr<ID3D11Device>	_GRPDEV, ComPtr<ID3D11DeviceContext> _DEVCTX) : GameObject(_GRPDEV, _DEVCTX) {}
Monster_Void::~Monster_Void() {}

HRESULT Monster_Void::Initialize_ProtoType() {
	...
	Component_Model->Load_FBXModel(L"../../Resource/Asset/Character/Monster/Monster_Void/Monster_Void.fbx", ANIMATION_TYPE::ANIMATION, Component_Animator);

	return S_OK;
}
HRESULT Monster_Void::Initialize(VOID* _ARG) {
	...
	Component_Animator->Play_Animation(static_cast<uint32_t>(MONSTER_VOID::WAKEUP), true);
	...
	Component_Transform->Set_WorldPosition(-20.5f, 0.f, 0.f);
...
VOID	Monster_Void::Update(CONST _float& _DT) {
...
VOID	Monster_Void::Late_Update(CONST _float& _DT){
...
HRESULT	Monster_Void::Render() {
	XMFLOAT4X4 ViewMatrix, ProjMatrix;

	XMFLOAT4 DissolveColor = { 165.f / 255.f, 38.f / 255.f, 176.f / 255.f, 1.f };
...
VOID Monster_Void::On_CollisionEnter(shared_ptr<GameObject> _ColliderOwner) {
	if (OBJDEAD) return;
...
VOID Monster_Void::On_CollisionStay(shared_ptr<GameObject> _ColliderOwner) {

}
VOID Monster_Void::On_CollisionExit(shared_ptr<GameObject> _ColliderOwner) {
	
}

VOID	Monster_Void::Generate_BehaviorTree() {
	BlackBoard		= BTBlackBoard::Create();
	BehaviorTree	= BehaviorTree_Void::Create(BlackBoard);
	...
	vector<uint32_t> AttackAnimIndexList = {
		static_cast<uint32_t>(MONSTER_VOID::ATTACK01),
		static_cast<uint32_t>(MONSTER_VOID::ATTACK02),
		static_cast<uint32_t>(MONSTER_VOID::ATTACK03)
	};
	...
	BlackBoard->Set_Value<_float>("MovementAmount", 0.75f);
	...
	auto DefaultStateSelector = make_unique<Selector>(BlackBoard, shared_from_this());
	{
		...
		DeathSequencer->AddNode(make_unique<VD_Play_DeathAnimation>(BlackBoard, shared_from_this(), Component_Transform, Component_Animator, static_cast<uint32_t>(MONSTER_VOID::DEATH)));
		DeathSequencer->AddNode(make_unique<VD_Play_DissolveEffect>(BlackBoard, shared_from_this(), static_pointer_cast<Monster_Void>(shared_from_this())));
	...
	}
	{
		...
		DamageSequencer->AddNode(make_unique<Play_AttackedAnimation>(BlackBoard, shared_from_this(), Component_Animator, Component_Transform, &HitCall, static_cast<uint32_t>(MONSTER_VOID::BEHIT_LEFT), 2.f));
	...
	auto DetectSelector = make_unique<Selector>(BlackBoard, shared_from_this());
	{
		...
		FirstDetectSequencer->AddNode(make_unique<Play_StagingAnimation>(BlackBoard, shared_from_this(), static_cast<uint32_t>(MONSTER_VOID::WAKEUP), 2.f));
		FirstDetectSequencer->AddNode(make_unique<Play_CombatStateAnimation>(BlackBoard, shared_from_this(), static_cast<uint32_t>(MONSTER_VOID::BATTLESTANCE)));
	...
	}
	{
		...
		OverDetectSequencer->AddNode(make_unique<VD_ResetToCombatNode>(BlackBoard, shared_from_this(), 2.f));
	...
	ComBatSequencer->AddNode(make_unique<Attack_TranslateNode>(BlackBoard, shared_from_this(), static_cast<uint32_t>(MONSTER_VOID::WALK_LEFT), 1.5f));
	ComBatSequencer->AddNode(make_unique<Attack_RotateNode>(BlackBoard, shared_from_this(), 3.f));
	ComBatSequencer->AddNode(make_unique<Attack_AnimationNode>(BlackBoard, shared_from_this(), AttackAnimIndexList, 1.5f));
	...
	RevertSequencer->AddNode(INVERTNODE(make_unique<Check_TargetDistance>(BlackBoard, shared_from_this(), PlayerTransform, 20.f)));
	{
		...
		RotateMoveSequencer->AddNode(make_unique<Rotate_CenterPoint>(BlackBoard, shared_from_this(), 1.f));
		RotateMoveSequencer->AddNode(make_unique<Move_CenterPoint>(BlackBoard, shared_from_this(), static_cast<uint32_t>(MONSTER_VOID::WALK_FRONT)));

		RevertSequencer->AddNode(move(RotateMoveSequencer));
	}
	RevertSequencer->AddNode(make_unique<VD_RevertNode>(BlackBoard, shared_from_this()));
...
VOID Monster_Void::Update_StateUI(CONST _float& _DT) {
...
unique_ptr<Monster_Void>Monster_Void::Create(ComPtr<ID3D11Device> _GRPDEV, ComPtr<ID3D11DeviceContext> _DEVCTX) {
	auto Instance = unique_ptr<Monster_Void>(new Monster_Void(_GRPDEV, _DEVCTX));
	if (FAILED(Instance->Initialize_ProtoType())) {
		MSG_BOX("Cannot Create Monster Void.");
		return nullptr;
	}
	return Instance;
}
shared_ptr<GameObject>	Monster_Void::Clone(VOID* _ARG) {
	auto Instance = shared_ptr<Monster_Void>(new Monster_Void(*this));
	if (FAILED(Instance->Initialize(_ARG))) {
		MSG_BOX("Cannot Clone Monster Void.");
		return nullptr;
	}
	return Instance;
}