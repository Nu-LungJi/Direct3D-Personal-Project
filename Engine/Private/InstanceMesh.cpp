...
InstanceMesh::InstanceMesh(ComPtr<ID3D11Device> _GRPDEV, ComPtr<ID3D11DeviceContext> _DEVCTX) : InstanceBuffer(_GRPDEV, _DEVCTX) {}
InstanceMesh::InstanceMesh(const InstanceMesh& _PRTOBJ) : InstanceBuffer(_PRTOBJ), MeshLoader(_PRTOBJ.MeshLoader), MeshName(_PRTOBJ.MeshName),
IndexList(_PRTOBJ.IndexList), TextureList(_PRTOBJ.TextureList) { }
InstanceMesh::~InstanceMesh() {}

HRESULT InstanceMesh::Initialize(VOID* _ARG) {
    VB_BufferCount = 2;
    VB_VertexSize = sizeof(VTXINS);
    ...
    VIB_VertexSize = sizeof(XMFLOAT4X4);
...
HRESULT InstanceMesh::Bind_TextureResource(shared_ptr<Shader> _Shader, const string& _SRVName, aiTextureType _TexType, uint32_t _TextureIndex) {
...
HRESULT InstanceMesh::UnBind_TextureResource(shared_ptr<Shader> _Shader, const string& _SRVName) {
    return _Shader->UnBind_ShaderResourceView(_SRVName);
}
VOID	InstanceMesh::Update_InstanceBuffer(const vector<XMMATRIX>& _InstancedTransform) {
    if (_InstancedTransform.empty()) return;

    VIB_InstanceCount = (uint32_t)_InstancedTransform.size();

    D3D11_MAPPED_SUBRESOURCE MRES;
    if (FAILED(DEVCTX->Map(VertexInstanceBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &MRES))) return;

    memcpy(MRES.pData, _InstancedTransform.data(), VIB_VertexSize * VIB_InstanceCount);
    DEVCTX->Unmap(VertexInstanceBuffer.Get(), 0);
}
HRESULT InstanceMesh::Bind_Resources() {
    if (FAILED(InstanceBuffer::Bind_Resources())) return E_FAIL;
    return S_OK;
}
HRESULT	InstanceMesh::Render_Buffer(uint32_t _InstanceCount) {
    if (_InstanceCount > 0)
        DEVCTX->DrawIndexedInstanced(VIB_IndexCount, _InstanceCount, 0, 0, 0);

    return S_OK;
}
HRESULT InstanceMesh::Decompose_MeshData(class MeshLoader* _Loader, const aiScene* _ModelData, aiMesh* _MeshData) {
    ...
    VIB_IndexCount = static_cast<uint32_t>(IndexList.size() );
...
HRESULT InstanceMesh::Decompose_MeshData(class MeshLoader* _Loader, const FB_StaticMesh::StaticMeshData* _MeshData) {
    MeshLoader = _Loader;
    ...
    VertexBufferDesc.ByteWidth = static_cast<uint32_t>(sizeof(VTXINS) * VertexList.size());//FB_VertexList->size();
    ...
    if (FAILED(GRPDEV->CreateBuffer(&ISBufferDesc, nullptr, VertexInstanceBuffer.GetAddressOf())))	return E_FAIL;

    return S_OK;
}
HRESULT InstanceMesh::Decompose_VertexData() {
    for (uint32_t IDX = 0; IDX < MeshData->mNumVertices; ++IDX) {
        VTXINS Vertex = { MeshData->mVertices[IDX].x, MeshData->mVertices[IDX].y, MeshData->mVertices[IDX].z };
...
HRESULT InstanceMesh::Decompose_IndexData() {
...
HRESULT InstanceMesh::Decompose_TextureData() {
...
HRESULT	InstanceMesh::Create_VertexBuffer() {
...
HRESULT	InstanceMesh::Create_IndexBuffer() {
...
HRESULT InstanceMesh::Create_InstanceBuffer() {
    D3D11_BUFFER_DESC				ISBufferDesc = {};
    ISBufferDesc.ByteWidth = VIB_InstanceCount * VIB_VertexSize;
    ISBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
    ISBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
...
flatbuffers::Offset<FB_StaticMesh::StaticMeshData> InstanceMesh::Compose_BinaryData(flatbuffers::FlatBufferBuilder& _Builder) {
...
shared_ptr<InstanceMesh> InstanceMesh::Create(ComPtr<ID3D11Device> _GRPDEV, ComPtr<ID3D11DeviceContext> _DEVCTX) {
    auto Instance = shared_ptr<InstanceMesh>(new InstanceMesh(_GRPDEV, _DEVCTX));
    if (FAILED(Instance->Initialize(nullptr))) {
        MSG_BOX("Cannot Create InstanceMesh");
        return nullptr;
    }
    return Instance;
}
