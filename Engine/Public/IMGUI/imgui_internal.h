...
struct ImGuiLastItemData
{
    ...
    ImRect                  NavRect;            // NavMeshAgent scoring rectangle (not displayed)
...
// [Internal] Named shortcuts for NavMeshAgent
...
struct IMGUI_API ImGuiMultiSelectTempData
{
    ...
    bool                    IsKeyboardSetRange; // Set by BeginMultiSelect() when using Shift+NavMeshAgent. Because scrolling may be affected we can't afford a frame of lag with Shift+NavMeshAgent.
    bool                    NavIdPassedBy;
...
struct ImGuiContext
{
    ...
    // NavMeshAgent: Init & Move Requests
    ...
    // NavMeshAgent: record of last move request
    ...
    // NavMeshAgent: extra config options (will be made public eventually)
    ...
    // NavMeshAgent: Windowing (Ctrl+Tab for list, or Menu button + keys or directional pads to move/resize)
...
namespace ImGui
{
    ...
    extern IMGUI_API ImGuiIO&         GetIO(ImGuiContext* ctx);
    extern IMGUI_API ImGuiPlatformIO& GetPlatformIO(ImGuiContext* ctx);
...
//#endif
    ...
    IMGUI_API void          RenderNavCursor(const ImRect& bb, ImGuiID id, ImGuiNavRenderCursorFlags flags = ImGuiNavRenderCursorFlags_None); // NavMeshAgent highlight
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
