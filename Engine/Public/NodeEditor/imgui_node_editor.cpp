...
void ed::EditorContext::SetNodePosition(NodeId nodeId, const ImVec2& position)
{
    ...
    if (node->m_Bounds.Min.x != position.x  || node->m_Bounds.Min.y != position.y)
    {
...
void ed::EditorContext::SetGroupSize(NodeId nodeId, const ImVec2& size)
{
    ...
    if (node->m_GroupBounds.GetSize().x != size.x && node->m_GroupBounds.GetSize().y != size.y)
    {
...
{
    Editor->MakeDirty(SaveReasonFlags::NavMeshAgent);
}
...
{
    return m_Path.size() > 1 && m_PathLength > 0.0f && (m_Link->m_Start.x == m_LastStart.x && m_Link->m_Start.y == m_LastStart.y) && (m_Link->m_End.x == m_LastEnd.x && m_Link->m_End.y == m_LastEnd.y);
}
...
bool ed::SizeAction::Process(const Control& control)
{
    if (m_Clean)
    {
        ...
        if (((m_SizedNode->m_Bounds.GetSize().x != m_StartBounds.GetSize().x) && (m_SizedNode->m_Bounds.GetSize().y != m_StartBounds.GetSize().y))  || (m_SizedNode->m_GroupBounds.GetSize().x != m_StartGroupBounds.GetSize().x) && m_SizedNode->m_GroupBounds.GetSize().y != m_StartGroupBounds.GetSize().y)
...
void ed::NodeBuilder::End()
{
    ...
    if ((m_CurrentNode->m_Bounds.GetSize().x != m_NodeRect.GetSize().x) && (m_CurrentNode->m_Bounds.GetSize().y != m_NodeRect.GetSize().y))
    {
